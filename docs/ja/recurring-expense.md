...
**定期支出**モジュールは、以下のような固定サイクルを持つ定期支出を管理するのに役立ちます：
- 電気、水道、ガス代
- インターネット、ケーブルTV
- 保険
- 学費
...
このモジュールは、設定したサイクルに基づいて自動的に**発生**を作成し、支払い期日をリマインドします。

## 2. 使用するタイミング

以下の場合にこのモジュールを使用します：
- スケジュールに沿った固定支出（週次、隔週、月次）
- 支払い実行の確認と追跡が必要
- 月次予算への自動計算を希望
...
- 支出を確認すると、アプリは現在の月の予算（存在する場合）を自動的に更新します
- 支出は予算の「定期支出」に計算されます
