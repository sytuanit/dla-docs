# Guía de Usuario - Español

¡Bienvenido a la guía de usuario de **Asistente de Vida Diaria** en español!

<p><strong>Vídeo de presentación</strong></p>
<div style="margin:0.75rem 0 1.25rem;max-width:min(360px,100%);">
  <div style="position:relative;width:100%;padding-bottom:177.78%;height:0;background:#111;border-radius:10px;overflow:hidden;">
    <iframe style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;"
      src="https://www.youtube.com/embed/uqO7-KRDVMk"
      title="YouTube"
      loading="lazy"
      referrerpolicy="strict-origin-when-cross-origin"
      allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
      allowfullscreen></iframe>
  </div>
</div>

## Lista de Módulos

### Gestión Financiera

- [Ingresos Recurrentes](./recurring-income.md) - Gestionar fuentes de ingresos regulares (salario, alquiler, etc.)
- [Ingresos Extra](./extra-income.md) - Registrar ingresos no recurrentes
- [Gastos Recurrentes](./recurring-expense.md) - Gestionar gastos recurrentes (servicios públicos, etc.)
- [Gastos Diarios](./daily-expense.md) - Registrar gastos diarios
- [Presupuesto](./budget.md) - Planificar y seguir presupuestos mensuales
- [Ahorros](./savings.md) - Gestionar cuentas de ahorro
- [Préstamos Bancarios](./bank-debt.md) - Gestionar préstamos y pagos de deuda
- [Objetivos Financieros](./financial-goals.md) - Establecer y seguir objetivos financieros

### Estilo de Vida

- [Ocasiones Especiales](./special-occasions.md) - Gestionar ocasiones especiales y listas de verificación de preparación
- [Lista de Tareas](./todo-list.md) - Gestionar tareas recurrentes y seguir el progreso de finalización

### Cocina y gastronomía

- [Recetas](./recipes.md) - Guardar y gestionar recetas, platos y colecciones
- [Planificación de menús](./menu-planning.md) - Planificar comidas semanales (desayuno / comida / cena), resumen semanal
- [Compras (lista)](./shopping-checklist.md) - Listas de compra; enlazables con pasos de preparación en Ocasiones especiales

### Informes

- [Informes](./reports.md) - Ver informes financieros detallados

### Configuración

- [Configuración](./settings.md) - Configurar aplicación, categorías, idioma, moneda
- [Copia de Seguridad y Restauración](./backup-restore.md) - Hacer copia de seguridad y restaurar datos
- [Términos de Servicio](./terms.md) - Términos y condiciones para usar la aplicación
- [Política de Privacidad](./privacy.md) - Política de privacidad y protección de datos

